...
- **Card de costos** (`.resultado-costos`): filas Costo compra → Costo producción → Comisión compra → **Costo total** (línea separadora sólida) → Com. venta (−ingreso) (línea separadora punteada, itálica, opacidad reducida). La fila "Com. venta" está visualmente separada del total para dejar claro que reduce el ingreso, no el costo. CSS: `.resultado-descuento-row` con `border-top: 1px dashed`, `opacity: 0.75`, `font-style: italic`.
