...
const { getDolares, DOLAR_FALLBACK, getGanado, getGanadoMock, getRosgan, getNoticias } = require("./cattle-api");
...
async function preciosHandler(req, res) {
  try {
    // Precalentar dólar y ROSGAN en paralelo; getGanado() reutiliza las cachés tibias
    const [dolar] = await Promise.all([getDolares(), getRosgan()]);
...
async function noticiasHandler(req, res) {
  try {
    res.json(await getNoticias());
  } catch {
    res.status(502).json({ ok: false, error: "No se pudieron obtener noticias" });
  }
}

module.exports = { dolarHandler, ganadoHandler, preciosHandler, rosganHandler, noticiasHandler };
